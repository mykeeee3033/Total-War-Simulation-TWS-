...
if (isServer) then {
	while {true} do {
		...
		_airframe1 = [getMarkerPos "logi_spawn_blufor", 140, "B_W_VTOL_01_vehicle_F", _crew1] call BIS_fnc_spawnVehicle;
...
"];
		...
		_wp2 setWaypointStatements ["true", "hint 'Egressing: Next flight will be in 1 hour.';"];

		// Waypoint 3: Plane returns to spawn, then deletes itself
		_wp3 = _crew1 addWaypoint [(getMarkerPos "logi_spawn_blufor"), 0];
