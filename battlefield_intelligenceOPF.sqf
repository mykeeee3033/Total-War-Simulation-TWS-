...
// Debug toggle for development. Keep disabled in live missions.
if (isNil "BATTLE_debug") then {
    BATTLE_debug = false;
};
...
BATTLE_fnc_trackOpforDeath = {
    ...
    // Fast type filter to avoid processing irrelevant entities
    ...
    if (BATTLE_debug) then {
        systemChat format ["[BATTLE Debug] OPFOR killed by %1 (Total: %2)", toUpper _killerType, _totalDeaths + 1];
    };
...
addMissionEventHandler ["EntityKilled", {
    ...
    // Fast pre-filter: only process unit categories this system cares about
    if (isNull _unit) exitWith {};
    if (!(_unit isKindOf "Man" || _unit isKindOf "LandVehicle" || _unit isKindOf "Air" || _unit isKindOf "Ship")) exitWith {};

    // Fast pre-filter: only OPFOR-side losses
    private _unitSide = side _unit;
    private _groupSide = side group _unit;
    if !(_unitSide == east || _groupSide == east) exitWith {};
