if (isServer) then {
    while { true } do {
        ...
        private _heli = [getMarkerPos "heli_spawn", 0, "B_Heli_Transport_03_F", _heliGroup] call BIS_fnc_spawnVehicle;

        // Wait a moment for the helicopter to initialize
        sleep 1;

        // Get the position of the cargo depot and spawn position
        private _cargoDepotPos = getMarkerPos "cargo_depot";
        private _spawnPos = getMarkerPos "heli_spawn";
        ...
        // Get all military markers within a specified radius (7000 meters)
        private _militaryMarkers = sectors_military select {
            private _markerPos = markerPos _x;
            (_markerPos distance _spawnPos) <= 7000 // Check if the marker position is within 7000 meters
        };
        ...
        if (count _militaryMarkers > 0) then {
            ...
            // Allow delivery to BLUFOR and Civilian sectors
            if (_ownership == GRLIB_side_friendly || _ownership == GRLIB_side_civilian) then {
            ...
            diag_log "No military markers found within 7000 meters!";
        };
