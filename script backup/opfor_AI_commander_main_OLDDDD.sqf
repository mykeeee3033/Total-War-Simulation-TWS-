 ...
 *
 * OPFOR commander support loop:
 * - Reads BLUFOR sightings from recon_intelligence_opf.sqf (RECON_OPF_spottedTargets)
 * - Stores normalized sighting data for commander decisions
 * - Uses HQ markers with prefix O_HQ_ (example: O_HQ_1)
 * - Spawns/reuses HQ artillery, spawns temporary OPFOR crew, fires mission, despawns crew
 * - Enforces cooldown between support actions
 */
...
if (isNil "OPFCOM_loopInterval") then { OPFCOM_loopInterval = 15; };
if (isNil "OPFCOM_supportCooldown") then { OPFCOM_supportCooldown = 60; }; // 1 minute
...
if (isNil "OPFCOM_mediumArtyClass") then { OPFCOM_mediumArtyClass = "O_MBT_02_arty_F"; };
...
if (isNil "OPFCOM_fireRounds") then { OPFCOM_fireRounds = 12; };
if (isNil "OPFCOM_fireSpread") then { OPFCOM_fireSpread = 120; };
if (isNil "OPFCOM_mediumFireRounds") then { OPFCOM_mediumFireRounds = 18; };
if (isNil "OPFCOM_mediumFireSpread") then { OPFCOM_mediumFireSpread = 140; };
if (isNil "OPFCOM_heavyFireRounds") then { OPFCOM_heavyFireRounds = 36; };
if (isNil "OPFCOM_heavyFireSpread") then { OPFCOM_heavyFireSpread = 450; };
...
// Artillery decision-control gates (anti-spam realism)
if (isNil "OPFCOM_contactMemory") then { OPFCOM_contactMemory = createHashMap; };
if (isNil "OPFCOM_minContactPersistence") then { OPFCOM_minContactPersistence = 30; }; // seconds
if (isNil "OPFCOM_contactMemoryMaxAge") then { OPFCOM_contactMemoryMaxAge = 300; }; // seconds
...
OPFCOM_fnc_isHQAvailable = {
	...
	private _hqOwner = [_hqMarker] call OPFCOM_fnc_getHQOwnership;
	_hqOwner == "OPFOR"
};
...
OPFCOM_fnc_selectPrimaryTarget = {
	...
	// Prioritization model inspired by battlefield_intelligenceOPF threat hierarchy.
	{
		private _t = _x;
		_t params ["_group", "_pos", "_lastSeen", "_class", "_size", "_status", "_mix", "_markerName"];

		// Hard rule: no artillery missions on air targets.
		if !(_class isEqualTo "air") then {
			private _persistence = [_t] call OPFCOM_fnc_getPersistenceSeconds;
			if (_persistence < OPFCOM_minContactPersistence) then {
				continue;
			};

			if !([_class, _size] call OPFCOM_fnc_passesThreatSizeGate) then {
				continue;
			};
			...
			if (_needsLossTrigger && {_lossesNear < OPFCOM_minLossesForIndirect}) then {
				continue;
			};
...
OPFCOM_fnc_selectArtilleryClassForTarget = {
	...
	if (_size <= OPFCOM_mediumTargetMax) exitWith {OPFCOM_mediumArtyClass};
	OPFCOM_heavyArtyClass
};
...
OPFCOM_fnc_getFireProfileForTarget = {
	...
	if (_size <= OPFCOM_mediumTargetMax) exitWith {[OPFCOM_mediumFireRounds, OPFCOM_mediumFireSpread, "medium"]};
	[OPFCOM_heavyFireRounds, OPFCOM_heavyFireSpread, "high"]
};
...
OPFCOM_fnc_fireMission = {
	params ["_gun", "_targetPos", ["_rounds", OPFCOM_fireRounds], ["_spread", OPFCOM_fireSpread]];

	private _ammoOptions = getArtilleryAmmo [_gun];
	if (count _ammoOptions == 0) exitWith {false};

	// Pick ammo type with the highest available rounds so salvos are sustainable.
	private _magInfo = magazinesAmmo _gun;
	private _bestAmmo = _ammoOptions select 0;
	private _bestCount = -1;
	{
		...
		private _available = 0;
		{
			...
			if (_magClass == _ammoClass) then {
				_available = _available + _rounds;
			};
		} forEach _magInfo;

		if (_available > _bestCount) then {
			_bestCount = _available;
			_bestAmmo = _ammoClass;
		};
	...
	while {_remaining > 0} do {
		private _batch = _remaining min 3;
		private _rndPos = [
		...
		private _fired = _gun doArtilleryFire [_rndPos, _bestAmmo, _batch];
		if (!_fired) then {
...
OPFCOM_fnc_executeSupportAction = {
	...
	if (!isNull _grp && {count _units > 0}) then {
		sleep 1.5;
		[_gun, _targetPos, _rounds, _spread] call OPFCOM_fnc_fireMission;
...
[] spawn {
	diag_log "[OPFCOM] OPFOR AI commander started";
	systemChat "[OPFCOM] OPFOR AI commander online";
	...
	publicVariable "OPFCOM_running";
	diag_log "[OPFCOM] OPFOR AI commander stopped";
};

publicVariable "OPFCOM_enabled";
publicVariable "OPFCOM_loopInterval";
publicVariable "OPFCOM_supportCooldown";
publicVariable "OPFCOM_lightArtyClass";
publicVariable "OPFCOM_mediumArtyClass";
publicVariable "OPFCOM_heavyArtyClass";
publicVariable "OPFCOM_lightTargetMax";
publicVariable "OPFCOM_mediumTargetMax";
publicVariable "OPFCOM_fireRounds";
publicVariable "OPFCOM_fireSpread";
publicVariable "OPFCOM_mediumFireRounds";
publicVariable "OPFCOM_mediumFireSpread";
publicVariable "OPFCOM_heavyFireRounds";
publicVariable "OPFCOM_heavyFireSpread";
publicVariable "OPFCOM_crewRetainTime";
publicVariable "OPFCOM_cachedSightings";
publicVariable "OPFCOM_minContactPersistence";
publicVariable "OPFCOM_baseScoreThreshold";
publicVariable "OPFCOM_escalationPoints";
publicVariable "OPFCOM_lossWindowSeconds";
publicVariable "OPFCOM_lossRadiusMeters";
publicVariable "OPFCOM_minLossesForIndirect";
